...
const Productdetails = () => {
  ...
  return isLoading ? (
    ...
    <div className="bg-gray-100 w-[100vw] pt-10">
      <div className="flex flex-col md:flex-row">
        <img
          className="object-contain max-h-[50vh] md:w-1/2"
          src={data?.product.photo}
          alt=""
        />
        <div className="mt-4 flex flex-col px-10 gap-5 md:w-1/2 justify-center">
          <h1 className="text-xl md:text-2xl">{data?.product.name}</h1>
          <p className="font-semibold md:text-xl">₹{data?.product.price}</p>
          <button
            className="bg-black md:text-xl rounded-xl px-3 text-white"
            onClick={() =>
              addToCartHandler({
